...
describe("read access allowed", function() {
  ...
  it("allows whitelisted urls", function() {
    ...
    assert.ok(
      readAllowed(url.parse("http://127.0.0.1/"), [{ hostname: "127.0.0.1" }])
    );
  });
  ...
  it("disallows not whitelisted urls", function() {
    ...
    assert.ok(
      !readAllowed(url.parse("http://wikipedia.org/"), [{ protocol: "https:" }])
    );
  });
  ...
  it("supports complex whitelists", function() {
    ...
    assert.ok(
      readAllowed(
        url.parse("ssh2.exec://user:pass@example.com:22/usr/local/bin/somecmd"),
        COMPLEX_WHITELIST
      )
    );
    assert.ok(
      readAllowed(
        url.parse("ssh2.exec://user:pass@google.com:22/usr/local/bin/somecmd"),
        COMPLEX_WHITELIST
      )
    );
    assert.ok(
      !readAllowed(
        url.parse("ssh2.exec://user:pass@example.com:22/usr/local/bin/vips"),
        COMPLEX_WHITELIST
      )
    );
    assert.ok(
      !readAllowed(
        url.parse("ssh2.exec://user:pass@example.com/usr/local/bin/somecmd"),
        COMPLEX_WHITELIST
      )
    );
  });
